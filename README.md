...
- 改进图片提取逻辑，使用标准的 WordPress API `get_attached_media()`
- 移除不必要的全局变量操作，避免副作用
- 优化代码兼容性和稳定性
...
### 2.0.2
- 优化筛选选项文字显示（"没有摘要"→"无摘要"、"没有特色图片"→"无特色图片"）
- 修复筛选功能不生效的问题
- 改进自动筛选逻辑和参数处理

### 2.0.1
- 优化特色图片生成流程，支持更多图片返回格式
- 增强图片文件扩展名识别，支持 JPEG/JPG/PNG/SVG
- 添加筛选失败时的错误提示和重试机制
- 优化用户界面反馈，使用 Toast 消息替代 alert
- 支持更多图片格式和自定义模型配置
- 新增图片描述生成和图片文件重命名功能
- 新增文章润色功能（标准、正式、轻松、创意四种风格）
- 新增文章排版功能（标准、博客、技术文档三种格式）
- 优化 AJAX 错误处理，提供更友好的错误提示
...
- 新增筛选功能，快速查找需要处理的文章
- 新增分页功能，支持大量文章管理
- 优化 AJAX 交互，避免 alert 弹窗
- 增强错误处理和用户反馈
- 新增文章排除功能（后续版本已移至管理页面）
- 改进 Token 统计和日志展示
